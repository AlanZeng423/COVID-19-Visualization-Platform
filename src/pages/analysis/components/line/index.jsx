...
export default function Radar() {
    ...
    const drawChart = () => {
		...
		const xAxis = d3.axisBottom(x)
    		.tickValues(data.provinceName.filter((_, i) => i % 2 === 0)); // 每隔 5 个显示一个标签
		...
		data.provinceName.forEach((province, i) => {
			svg.append("circle")
				...
				.on("mouseover", function() {
					...
					tooltip.transition().duration(200).style("opacity", 1);
					tooltip.html(`省份: ${xValue}<br>确诊比例: ${yValue1}<br>迁入比例: ${yValue2}`)
						.style("left", (x(xValue) + x.bandwidth() / 2) + "px")
						.style("top", (y(yValue1) - 28) + "px");
				})
				.on("mouseout", function() {
		...
		// 图例数据
		const legendData = [
			{ name: "迁入比例", color: "#F7DC6F" },
			{ name: "确诊比例", color: "#F1948A" }
		];
